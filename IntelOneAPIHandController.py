from onewire.oneapi import io
from onewire.oneapi import pwm
...
for pin in servo_pins:
    pwm_obj = pwm.PWM(gpio, pin)
    pwm_obj.set_frequency(50)
    pwm_obj.enable()
    pwms[pin] = pwm_obj
...
def set_angle(pwm_obj, angle):
    ...
    pwm_obj.set_duty_cycle(duty_cycle)
    time.sleep(0.3)
    pwm_obj.set_duty_cycle(0)
...
servo_speed = 20  # Degrees per keyboard press
...
try:
    while True:
       ...
       # Update servo angles based on keyboard input
        ...
        # Set servo angles
        for pin, angle in zip(servo_pins, servo_angles):
